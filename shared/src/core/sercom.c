...
void SERCOM4_Handler(void) {
    if (SERCOM4->USART.INTFLAG.bit.RXC != 0) {
        if (ring_buffer_write(&ringBuffer,
                              (uint8_t)(SERCOM4->USART.DATA.reg))) {
            // How to handle?
        }
    }
}
...
void sercom_uart_init(Sercom *sercom, unsigned long baud) {
    ...
    if (sercom == SERCOM4) {
        PORT->Group[2].WRCONFIG.reg =
            (uint32_t)((1 << 8) | (1 << 9) | PORT_WRCONFIG_PMUXEN |
    ...
    sercom->USART.CTRLA.reg =
        SERCOM_USART_CTRLA_DORD | SERCOM_USART_CTRLA_MODE(1) |
        SERCOM_USART_CTRLA_TXPO(1) | SERCOM_USART_CTRLA_RXPO(3);
...
bool uart_data_available(void) { return !ring_buffer_empty(&ringBuffer); }
